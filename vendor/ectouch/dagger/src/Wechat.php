...
class Wechat extends SDK
{
    ...
    /**
     * app支付 统一下单接口 （默认交易类型APP)
     * @param array $arr 请求下单参数
     * @param boolean $jsSign false返回下单参数，ture返回H5签名
     * @return boolean|array
     */
    public function appPayOrder($arr = array(), $jsSign = false)
    {
        if (empty($arr)) return false;
        $arr['device_info'] = isset($arr['device_info']) ? $arr['device_info'] : "WEB";
        $arr['fee_type'] = isset($arr['fee_type']) ? $arr['fee_type'] : "CNY";
        $arr['trade_type'] = isset($arr['trade_type']) ? $arr['trade_type'] : "APP";
        $arrdata = $this->getPaySign($arr);
        $xmldata = $this->xml_encode($arrdata);
        $result = $this->http_post(self::PAY_PREFIX . self::PAY_UNIFIEDORDER, $xmldata);
        if ($result) {
            ...
            //生成微信签名
            return $jsSign == false ? $json : $this->getPayAppSign($json['prepay_id']);
        }
        return false;
    }
     ...
     * app支付二次签名
     * @param array $arr 需要签名的数据
     * @param int $str prepayid:预支付交易会话标识
     * @return array|bool 返回签名字串
     */
    public function getPayAppSign($str)
    {
        if (empty($str)) return false;
        $arr = array();
        $arr['appid'] = $this->appid;
        ...
        $arr["partnerid"]  = $this->mch_id;
        $arr['prepayid'] = $str;
        $arr['timestamp'] = time();
        $paySign = $this->getPaySignature($arr);
        $arr['sign'] = $paySign;
        $arr["packagestr"] = "Sign=WXPay";
        return $arr;
    }
