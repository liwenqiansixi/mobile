...
function get_wechat_user_info($id = 0)
{
    ...
    if(is_wechat_browser() && is_dir(APP_WECHAT_PATH)){
        $sql = 'SELECT u.user_name, u.nick_name, u.user_picture, wu.headimgurl, wu.nickname FROM ' . $GLOBALS['ecs']->table('users') . 'AS u ' . ' LEFT JOIN '  . $GLOBALS['ecs']->table('wechat_user') . 'AS wu ' .
        ' ON u.user_id = wu.ect_uid ' .
        " WHERE u.user_id = '$id' ";
    }else{
         // users
...
function set_ru_id()
{
    if (is_dir(APP_WECHAT_PATH) && !empty($_GET['ru_id'])) {
        ...
        cookie($cookiekey, intval($_GET['ru_id']), 3600 * 24); // 过期时间为 1 天
    }
}
